...
#define OUTNUM 54   // "110110"; -- OUTNUM Rx     -- Imprime o valor numerico decimal de Rx       Format: < inst(6) | Rx(3) | xxxxxxx >
...
// ============================================================
// Cada entrada da BHT (Branch History Table) armazena um
// contador de 2 bits com 4 estados:
//
//   00 (0) = Fortemente Nao-Tomado  (Strongly Not Taken)
//   01 (1) = Fracamente Nao-Tomado  (Weakly Not Taken)
//   10 (2) = Fracamente Tomado      (Weakly Taken)
//   11 (3) = Fortemente Tomado      (Strongly Taken)
//
// Transicoes:
//   Branch TOMADO:     estado++ (satura em 3)
//   Branch NAO-TOMADO: estado-- (satura em 0)
//
// Predicao: se estado >= 2 -> preve TOMADO; senao -> preve NAO-TOMADO
//
// A tabela e indexada pelos bits baixos do PC do branch (PC % BHT_SIZE),
// permitindo rastrear ate BHT_SIZE branches distintos simultaneamente.
// ============================================================

#define BHT_SIZE 64          // Numero de entradas na Branch History Table
                              // (potencia de 2 para que o indice seja apenas PC & mask)
#define BP_FORTE_NAO  0      // Fortemente Nao-Tomado
#define BP_FRACO_NAO  1      // Fracamente Nao-Tomado
#define BP_FRACO_SIM  2      // Fracamente Tomado
#define BP_FORTE_SIM  3      // Fortemente Tomado
#define BP_LIMIAR     2      // Estado >= limiar => preve TOMADO

typedef struct {
    unsigned char contador[BHT_SIZE]; // Contadores de 2 bits (0..3)
    // Estatisticas de desempenho
    unsigned int total;
    unsigned int acertos;
} BranchPredictor;

BranchPredictor bp; // Instancia global do preditor

// Inicializa o preditor: todos os contadores comecam em estado "Fracamente Nao-Tomado"
// (estado 1), que e um ponto neutro — o preditor nao tem opiniao formada ainda.
void bp_init(void) {
    int i;
    for (i = 0; i < BHT_SIZE; i++)
        bp.contador[i] = BP_FRACO_NAO;
    bp.total   = 0;
    bp.acertos = 0;
}

// Retorna 1 se o preditor acha que o branch SERA tomado, 0 caso contrario.
// pc_branch: endereco da instrucao de branch (usado como indice na BHT).
int bp_prediz(int pc_branch) {
    int idx = pc_branch % BHT_SIZE;
    return (bp.contador[idx] >= BP_LIMIAR) ? 1 : 0;
}

// Atualiza o preditor apos o resultado real do branch ser conhecido.
// tomado: 1 se o branch foi efetivamente tomado, 0 caso contrario.
// pc_branch: mesmo PC usado em bp_prediz().
void bp_atualiza(int pc_branch, int tomado, int previsao) {
    int idx = pc_branch % BHT_SIZE;

    // Compara previsao com resultado real
    bp.total++;
    if (previsao == tomado)
        bp.acertos++;

    // Atualiza o contador saturante
    if (tomado) {
        if (bp.contador[idx] < BP_FORTE_SIM)
            bp.contador[idx]++;
    } else {
        if (bp.contador[idx] > BP_FORTE_NAO)
            bp.contador[idx]--;
    }
}

// Imprime um relatorio de desempenho do preditor
void bp_relatorio(void) {
    printf("\n========================================\n");
    printf("  RELATORIO DO PREDITOR DE BRANCH\n");
    printf("========================================\n");
    printf("  Branches encontrados : %u\n", bp.total);
    printf("  Previsoes corretas   : %u\n", bp.acertos);
    if (bp.total > 0)
        printf("  Taxa de acerto       : %.1f%%\n",
               100.0 * bp.acertos / bp.total);
    else
        printf("  Taxa de acerto       : N/A (nenhum branch)\n");
    printf("========================================\n");
}
// ============================================================
...
int main()
{
	...
	// Variaveis do preditor de branch
	int bp_pc=0;       // PC no momento do fetch do branch (antes do IncPC)
	int bp_prev=0;     // Previsao feita pelo preditor para o branch atual
	int bp_tomado=0;   // Resultado real: 1=tomado, 0=nao-tomado

	le_arquivo();
	bp_init(); // Inicializa o preditor de branch
	...
	switch(state)
	{
		...
		case STATE_DECODE:
			...
			switch(opcode){
				...
				case OUTNUM:
					// Imprime o valor numerico decimal do registrador Rx
					printf("%d", (int)reg[rx]);
					// -----------------------------
					state=STATE_FETCH;
					break;
				...
				case JMP:
					...
					// --- PREDITOR DE BRANCH ---
					// Salva o PC do branch (PC ja foi incrementado apos o FETCH,
					// entao voltamos 1 para ter o endereco real da instrucao JMP)
					bp_pc  = PC - 1;
					bp_prev = bp_prediz(bp_pc); // Consulta a previsao ANTES de executar
					...
					{ // PC = MEMORY[PC];
						...
						RW = 0;
						LoadPC = 1;
						bp_tomado = 1; // Branch foi TOMADO
					}
					else
					{
						//PC++;
						IncPC = 1;
						bp_tomado = 0; // Branch NAO foi tomado
					}

					// Atualiza o preditor com o resultado real
					bp_atualiza(bp_pc, bp_tomado, bp_prev);
					// -----------------------------
					state=STATE_FETCH;
					break;
		...
		case STATE_HALTED:
			printf("\n");
			bp_relatorio(); // Exibe estatisticas do preditor ao final da execucao
			key = getchar();
			if (key == 'r') { bp_init(); goto inicio; } // Reinicia preditor junto com o processador
			if (key == 'q') goto fim;
			break;
